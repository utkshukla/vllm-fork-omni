...
def dynamic_preprocess(image, min_num=1, max_num=12, image_size=384, mask_size=27):
    target_aspect_ratio, target_height, target_width = _find_target_aspect_ratio(
        image, image_size, max_num, min_num
    )
    padding_height, padding_width = _get_padding_size(image, target_height, target_width)
    ...
    image = T.functional.resize(image, [new_size[1], new_size[0]],)

    resized_img = T.functional.pad(image, [0, 0, padding_width, padding_height], fill=[255,255,255])
...
def preprocess(images, dynamic_hd_size, vit_resolution, vit_patch_size):
    ...
    image_transformed = [pad_to_max_num_crops(im, max_crops) for im in hd_images_reshape]
    image_transformed = torch.stack(image_transformed, dim=0)
    mask_transformed = [pad_mask_to_max_num_crops(mask, max_crops) for mask in hd_masks_reshape]
...
class PhiOImageEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # load weights
        self.load_weights(model_dir, num_hidden_layers=-1)

        # DEBUG
        self.unit_test()

    def load_weights(self, model_dir, num_hidden_layers=-1):

        weights = {}
        for weight_index in range(1,4):
            weights.update(load_file(os.path.join(model_dir, f"model-0000{weight_index}-of-00003.safetensors")))

        img_emb = {}
        for k, v in weights.items():
            if k.startswith("model.embed_tokens_extend.image_embed"):
                img_emb[".".join(k.split('.')[3:])] = v

        missing, unexpected = self.load_state_dict(img_emb, strict=False)
        logger.debug(f"-------------- missing: {missing}") # missing: []
        logger.debug(f"-------------- unexpected: {unexpected}") # unexpected: ['post_layernorm.bias', 'post_layernorm.weight']
        # exit(0)

    def unit_test(self):
        return
        d = torch.load("examples/hf.pt")
        # dict_keys(['input_embeds', 'image_sizes', 'img_set_tensor'])
        pixel_values = d["input_embeds"]
        image_attention_mask = d["image_attention_mask"]
        image_sizes = d["image_sizes"]

        outputs = self.forward(pixel_values,
                              image_sizes,
                              image_attention_mask)

        outputs_gt = d["img_set_tensor"]

        # # check output parity
        assert len(outputs_gt) == len(outputs), f"{len(outputs_gt)} {len(outputs)}"
        print("testing SigLIP parity")
        for output_gt, output in zip(outputs_gt, outputs):
            error = output_gt - output
            print(f"mean err: {torch.mean(torch.abs(error))}") # tensor(0.0344
            print(f"mean ground truth abs: {torch.mean(torch.abs(output_gt))}") # tensor(0.4590
    ...
    def forward(self,
        ...
        # assert num_images == 1, "Currently only support single image"  # TODO debug multi-image
        bs = num_images
...
def audio_feature_extractor() -> LogFbankProcessor:
    ...
    # the audio featues from the sound file
    # LRU cache ensures that we only make one copy
    return LogFbankProcessor()


@lru_cache
def _get_audio_placeholder_token_id_candidates(
    model_config: ModelConfig,
    idx: int,
) -> List[List[int]]:
    assert idx > 0, "Audio token index must be greater than 0"

    tokenizer = cached_get_tokenizer(model_config.tokenizer)

    # This is used when the audio token is at the start of the string
    start_candidate = tokenizer.encode(
        f"<|audio_{idx}|>", add_special_tokens=False
    )

    # This is used when the audio token is in the middle of the string
    ...
    a_token_id_, *middle_candidate = tokenizer.encode(
        f"a<|audio_{idx}|>", add_special_tokens=False
    )
    assert a_token_id == a_token_id_, "Unexpected tokenization"

    return [start_candidate, middle_candidate]


def _compute_num_image_tokens(
    image, dynamic_hd_size, vit_image_size, vit_patch_size, token_compression_factor
):
    """
...
def _get_audio_id_to_input_ids(prompt, audios, ctx: InputContext):
    if len(audios) == 0:
        return {}
    ...
    audio_ids = re.findall(AUDIO_TOKEN_PATTERN, prompt)
    ...
    assert tuple(audio_ids) == tuple(
        range(1, len(audio_ids) + 1)
    ), "Audio ids are not in order!"
    audio_id_to_input_ids = {
        f"<|audio_{audio_id}|>": [_AUDIO_PLACEHOLDER_TOKEN_ID] * audio_embed_size
...
def _get_image_id_to_input_ids(images, prompt, ctx: InputContext):
    ...
    assert len(image_ids) == len(set(image_ids)), "Duplicate image tokens in prompt"
    assert len(images) == len(image_ids), "Number of images and image tokens in prompt do not match"

    # NOTE the following assertion is not strictly necessary
    assert tuple(image_ids) == tuple(range(1, len(image_ids) + 1)), "Image ids are not in order"
    ...
    image_id_to_input_ids = {
        ...
        for image_id, num_tokens in zip(image_ids, image_token_numbers)
    }
...
def input_processor_for_phio(
    ...
    Args:
        ...
        inputs (DecoderOnlyInputs): The inputs (e.g. prompt, prompt_token_ids) to process.
    ...
    assert prompt is not None, "Prompt is required for multimodal inputs"
    ...
    if 'audio' in multi_modal_data:
        ...
        audio_id_to_input_ids = _get_audio_id_to_input_ids(prompt, audios, ctx)
    else:
        audio_id_to_input_ids = {}
    ...
    if 'image' in multi_modal_data:
        ...
        image_id_to_input_ids = _get_image_id_to_input_ids(images, prompt, ctx)
    else:
        image_id_to_input_ids = {}

    pattern = r"(<\|image_\d+\|>|<\|audio_\d+\|>)"
    prompt_chunk_strings = re.split(pattern, prompt)
    ...
    for prompt_chunk_string in prompt_chunk_strings:
        if re.match(IMAGE_TOKEN_PATTERN, prompt_chunk_string):
            ...
            input_ids.extend(tokenizer(prompt_chunk_string).input_ids)
...
def dummy_data_for_phi3s(
    ...
    to the audio embed size of the _AUDIO_MAX_SOUNDFILE_SIZE.
    ...
    seq_data = SequenceData.from_prompt_token_counts(
        ...
        (0, seq_len - audio_feature_size * audio_count - total_image_tokens),
    )
    mm_data = {
...
def input_mapper_for_phi3s(ctx: InputContext, data: object) -> MultiModalInputs:
    ...
    Note that the incoming audio data (each entry in `data`) is a tuple of the audio data
    and the sampling frequency (e.g. from soundfile.read).
...
class PhiOForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal, SupportsPP):
    ...
    # QKVParallelLinear, RowParallelLinear, MergedColumnParallelLinear, RowParallelLinear
    ...
    def _audio_features_to_embeddings(
        ...
        """
        Convert audio features to embeddings, which are used as input to the model (via
        `inputs_embeds`).

        Args:
            input_ids (torch.Tensor): Input IDs (the prompt in this case).
            input_features (list[torch.Tensor]): Input features (the audio embeddings).
            audio_input_sizes (list[torch.Tensor]): Audio input sizes (the audio embed lengths to use for
                padding the audio placeholder token in the input prompt IDs).
        """
        # The audio projection can either be a single linear or Sequential, so handle
        # both cases
        ...
        kwargs = {"wte": self.embed_tokens, 'audio_projection_mode': audio_projection_mode}
    ...
    def _parse_and_validate_image_input(
        ...
        if isinstance(pixel_values, list):
            ...
            # each tensor has dimension: num_img_per_example, num_hd_patches, channels, height, width
            ...
            # dimension: batch_size, num_img_per_example, num_hd_patches, channels, height, width
            # we flatten first 2 dims to make it a single large batch for SigLIP Encoder
            ...
            raise ValueError("Incorrect image inputs")
    ...
    def merge_image_features_to_inputs_embeds(
        ...
        position_tuple = (input_ids == _IMAGE_PLACEHOLDER_TOKEN_ID).nonzero(as_tuple=True)

        assert all([t.shape[0] == 1 for t in image_set_tensors]), 'img_set_tensor should have shape (1, N_tokens, C)'
        # Shape: (merged_N_tokens, C)
        image_set_tensor = torch.cat(image_set_tensors, dim=1).squeeze(0)
        image_set_tensor = image_set_tensor.to(inputs_embeds.dtype).to(inputs_embeds.device)
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]) -> None:
        ...
        for name, weight in weights.items():
            ...
            audio_proj_4v = "model.embed_tokens_extend.audio_embed.audio_projection.vision"
            ...
            # NOTE: this is deal with LoRA injection, where `base_layer` remains as the original
            # layer in the model
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        if intermediate_tensors is not None:
            ...
            # Each entry in this is a pair of audio_features and audio_embed lengths
            ...
            if has_image or has_audio:
                # multi-modal input, we have set inputs_embeds properly in previous steps
                input_ids = None
            else:
